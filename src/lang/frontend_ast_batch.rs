...
impl PredicateBatches {
    ...
    /// Build the full statement list for an OR predicate application.
    /// OR predicates expect statements for ALL branches, with Statement::None for non-chosen branches.
    fn build_or_statements(
        chosen_branch: usize,
        chosen_statements: Vec<Statement>,
        children: &[super::frontend_ast_lift::StatementStructure],
    ) -> Vec<Statement> {
        let mut result = Vec::new();
        for (i, child) in children.iter().enumerate() {
            if i == chosen_branch {
                result.extend(chosen_statements.iter().cloned());
            } else {
                // Pad non-chosen branches with Statement::None
                let count = child.native_count();
                for _ in 0..count {
                    result.push(Statement::None);
                }
            }
        }
        result
        ...
        E: From<MultiOperationError>,
    {
        ...
        let input_children = match input {
            ...
            PredicateInput::Or { branch, input } => {
                // OR input - handle two cases:
                // 1. Top-level OR predicate: structure has multiple elements (the OR's branches)
                // 2. Nested OR: structure has a single Nested element wrapping the OR

                // Case 1: Top-level OR predicate (structure contains the OR's branches directly)
                if structure.len() > 1 {
                    if branch >= structure.len() {
                    ...
                    // Recursively flatten with just the chosen branch's structure
                    return self.flatten_tree_input(*input, &[structure[branch].clone()], apply_op);
                }

                // Case 2: Nested OR (structure is a single Nested element)
                if structure.len() != 1 {
                    return Err(MultiOperationError::StructureMismatch { position: 0 }.into());
                }
                match &structure[0] {
                    StatementStructure::Nested {
                        ...
                        // Build full statement list with Statement::None for non-chosen branches
                        let all_statements =
                            Self::build_or_statements(branch, chosen_statements, children);
                        // Apply the OR predicate with all statements
                ...
                StatementStructure::Nested {
                    ...
                    match (child_input, conjunction_type) {
                        ...
                        (
                            ...
                            // Recursively flatten the chosen branch
                            let chosen_statements = self.flatten_tree_input(
                                *input,
