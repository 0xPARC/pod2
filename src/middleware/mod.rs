...
impl Default for Params {
    fn default() -> Self {
        Self {
            ...
            max_statements: 48 * 2 + 2,
            max_public_statements: 8,
            num_public_statements_hash: 16,
            max_statement_args: 5,
            max_operation_args: 5,
            max_custom_predicate_batches: 4 * 2 + 2,
            max_custom_predicate_verifications: 8 * 2 + 2,
            max_custom_predicate_arity: 5,
            max_custom_predicate_wildcards: 8 * 2 + 2,
            max_custom_batch_size: 4 * 2 + 2,
            max_merkle_proofs_containers: 20 * 2 + 2,
            max_merkle_tree_state_transition_proofs_containers: 6 * 2 + 2,
            max_depth_mt_containers: 32,
            max_depth_mt_vds: 6, // up to 64 (2^6) different pod circuits
            max_public_key_of: 2 * 2 + 2,
            max_signed_by: 4 * 2 + 2,
        }
    }
}
