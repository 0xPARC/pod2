...
impl MultiPodSolution {
    ...
    pub fn validate(&self, input: &SolverInput) -> Vec<String> {
        ...
        for &pub_idx in input.output_public_indices {
            // The statement must be either proved locally or available from
            // a parent pod (forwarded D statements).
                    ...
                    match dep {
                        ...
                        StatementSource::External(ext) => {
                            ...
                            let direct = ext_pod_idx
                                ...
                                .unwrap_or(false);
                            if !direct {
                                // Check if forwarded via a parent's public external premises.
                                let ext_prem_idx =
