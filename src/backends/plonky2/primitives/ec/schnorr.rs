...
use crate::{
    backends::plonky2::{
        ...
        primitives::ec::{
            ...
            curve::{CircuitBuilderElliptic, CircuitBuilderEllipticNew, PointTarget, WitnessWriteCurve, GROUP_ORDER},
        },
        Error,
    },
    middleware::RawValue,
...
impl SignatureTarget {
    pub fn verify(
        ...
        let r = builder.linear_combination_points_new(&sig1_bits, &sig2_bits, &g, public_key);
        ...
        match e_hash[0] {
            ...
            Target::VirtualTarget { index } => println!("e_hash[0]: virtual target with index {}", index),
        }
