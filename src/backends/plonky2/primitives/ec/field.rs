...
// /// Target type modelled on Extension field tensor OEF.
// #[derive(Debug, Clone, Serialize, Deserialize)]
// pub struct OEFExtTarget<const D: usize, const DEG: usize, NNF: OEF<DEG>> {
//     #[serde(with = "serde_arrays")]
//     pub components: [ExtensionTarget<D>; DEG],
//     _phantom_data: PhantomData<NNF>,
// }

// impl<const D: usize, const DEG: usize, NNF: OEF<DEG>> OEFExtTarget<D, DEG, NNF> {
//     pub fn new(components: [ExtensionTarget<D>; DEG]) -> Self {
//         Self {
//             components,
//             _phantom_data: PhantomData,
//         }
...
// impl<const D: usize, const DEG: usize, NNF: OEF<DEG>> Default for OEFExtTarget<D,DEG, NNF> {
//     fn default() -> Self {
//         Self::new([ExtensionTarget::default(); DEG])
//     }
// }

// impl<const DEG: usize, NNF: OEF<DEG> + FieldExtension<DEG, BaseField = F>>
//     CircuitBuilderNNF<F, D, NNF, OEFExtTarget<D, DEG, NNF>> for CircuitBuilder<F, D>
// {
//     fn add_virtual_nnf_target(&mut self) -> OEFExtTarget<D,DEG, NNF> {
//         OEFExtTarget::new(array::from_fn(|_| self.add_virtual_extension_target()))
//     }
//     fn nnf_constant(&mut self, x: &NNF) -> OEFExtTarget<D,DEG, NNF> {
//         unimplemented!()
//     }
//     fn nnf_add(&mut self, x: &OEFExtTarget<D, DEG, NNF>, y: &OEFExtTarget<D, DEG, NNF>) -> OEFExtTarget<D, DEG, NNF> {
//         let sum_targets = std::iter::zip(&x.components, &y.components)
//             .map(|(a, b)| self.add_extension(*a, *b))
//             .collect::<Vec<_>>();
//         OEFExtTarget::new(std::array::from_fn(|i| sum_targets[i]))
//     }
//     fn nnf_sub(&mut self, x: &OEFExtTarget<D,DEG, NNF>, y: &OEFExtTarget<D,DEG, NNF>) -> OEFExtTarget<D, DEG, NNF> {
//         let sub_targets = std::iter::zip(&x.components, &y.components)
//             .map(|(a, b)| self.sub_extension(*a, *b))
//             .collect::<Vec<_>>();
//         OEFExtTarget::new(std::array::from_fn(|i| sub_targets[i]))
//     }
//     fn nnf_mul(&mut self, x: &OEFExtTarget<D, DEG, NNF>, y: &OEFExtTarget<D, DEG, NNF>) -> OEFExtTarget<D, DEG, NNF> {
...
//         OEFExtTarget::new(array::from_fn(|i| outputs[i]))
//     }
//     fn nnf_div(&mut self, x: &OEFTarget<DEG, NNF>, y: &OEFTarget<DEG, NNF>) -> OEFTarget<DEG, NNF> {
//         let one = self.nnf_one();
//         // Determine denominator inverse witness.
