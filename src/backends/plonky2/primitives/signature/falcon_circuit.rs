//! This file implements the Plonky2 circuit which verifies the Falcon
//! signatures (https://falcon-sign.info). Compatible with the signatures
//! generated by signature.rs.
...
/// This gadget implements the Plonky2 circuit which verifies the Falcon
/// signatures (https://falcon-sign.info). Compatible with the signatures
/// generated by signature.rs.
...
impl SignatureVerifyTarget {
    pub fn build(builder: &mut CircuitBuilder<F, D>) -> Result<Self> {
        ...
        // not using lookup since doing the assert_less it's a small amount of
        // gates in the context of the logic
        assert_less::<63>(builder, sum_norms, norm_bound);
...
/// Operations for the Falcon-512 field (modulus 12289).
...
/// check if x<y, returning a BoolTarget indicating if the check was fullfilled
/// or not.
pub fn is_less<const NUM_BITS: usize>(
    ...
    // we follow the same approach as in circomlib:
    // https://github.com/iden3/circomlib/tree/0a045aec50d51396fcd86a568981a5a0afb99e95/circuits/comparators.circom#L89

    // x + 2^n
    let x_2n = builder.add_const(x, F::from_canonical_u64((1 << NUM_BITS) as u64));
    let res = builder.sub(x_2n, y);
...
/// Witness hint generator for decomposing the limbs, used at the method
/// `assert_less_maxq`.
/// v = \sum limb_i * base^i, where base=falcon prime.
#[derive(Debug, Default)]
...
// Note: this method is an exploration to reduce the amount of gates from
// `assert_less_maxq`, but it's current logic is not secure.
fn assert_less_maxq_WIP(builder: &mut CircuitBuilder<F, D>, falcon_lut_i: usize, v: Target) {
    // TODO WARNING: this check needs to be done differently, current logic is
    // not secure due step 4.
    ...
    // 2. set limbs := decompose_into_limbs(v)
    let limbs = builder.add_virtual_target_arr::<4>();
    builder.add_simple_generator(LimbsDecompGenerator { v, limbs });

    // 3. assert that v == from_limbs(limbs), hence the hinted witnes through
    //    the SimpleGenerator is correct
...
fn assert_less_maxq(builder: &mut CircuitBuilder<F, D>, v: Target) {
    let max_q = builder.constant(F::from_canonical_u64(MAX_Q));
    assert_less::<63>(builder, v, max_q);
}
...
pub mod tests {
    ...
    #[test]
    fn test_assert_less_maxq() -> Result<()> {
        // should pass (true)
        let v: F = F::from_canonical_u64(0_u64); // in range
        test_assert_less_maxq_opt(v, true)?;
        dbg!("AAA0");

        let v: F = F::from_canonical_u64(MAX_Q - 1); // in range
        test_assert_less_maxq_opt(v, true)?;
        dbg!("AAA1");

        let v: F = F::from_canonical_u64(1500732034968603 as u64); // in range
        test_assert_less_maxq_opt(v, true)?;

        // should not pass (false)
        let v: F = F::from_canonical_u64(MAX_Q * 2 - 100); // overflows max_q
        dbg!(v);
        test_assert_less_maxq_opt(v, false)?;
        dbg!("AAA2");

        // set all the limbs to p (falcon field), substract 1
        let v: F = F::from_canonical_u64(22808751752994819 as u64); // overflows max_q
        test_assert_less_maxq_opt(v, false)?;

        Ok(())
    }
    fn test_assert_less_maxq_opt(v: F, expect_pass: bool) -> Result<()> {
        let config = CircuitConfig::standard_recursion_config();
        let mut builder = CircuitBuilder::<F, D>::new(config);
        let mut pw = PartialWitness::<F>::new();
        ...
        assert_less_maxq(&mut builder, v_targ);
    ...
    fn test_falcon_gadget() -> Result<()> {
        ...
        let signature_targ = SignatureVerifyTarget::build(&mut builder)?;
