...
import type { Value, Dictionary, Set as PodSet, Array as PodArray, SignedPod } from "@/types/pod2";
import {
  signPod,
  type SignPodRequest,
...
const CreateSignedPodDialog: React.FC<CreateSignedPodDialogProps> = ({
  ...
  const [privateKeyError, setPrivateKeyError] = useState<string | undefined>();
  ...
  const convertUiValueToPodValue = (value: any, type: ValueTypeName): Value => {
    switch (type) {
      ...
      case 'Array':
        ...
        return value.map((item: PodCollectionItem) => convertUiValueToPodValue(item.value, item.type));
      ...
      case 'Dictionary':
        ...
        for (const item of value as PodDictionaryItem[]) {
          ...
          keys.add(key);
          dict[key] = convertUiValueToPodValue(item.value, item.type);
        }
  ...
  const handleSign = async () => {
    ...
    try {
      const signedPodData = await signPod(requestPayload);
